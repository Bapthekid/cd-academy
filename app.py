...
"""
    ...
    def send_async_email(application, message, data):
        with application.app_context():
            ...
            # Quick connectivity preflight: DNS + TCP
            try:
                if not can_connect_smtp(host, port, timeout=timeout):
                    logger.warning(f"SMTP server {host}:{port} unreachable; attempting SendGrid fallback if configured.")
            ...
            # As a last resort, attempt SendGrid before queuing
            if send_via_sendgrid(data):
                return
            queue_email_data(data)
